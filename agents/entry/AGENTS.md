...
**你期望的工作流：用户只介入一次，后续全部自动化。**

**实现方式：**

1. **识别任务类型**
   - 快速调研 → PM → 调研员 → 文档
   - 完整开发 → PM → 产品 → 架构 → 开发 → 测试 → 文档
   - 单纯开发 → PM → 开发 → 测试 → 文档

2. **使用 agent-team 工具自动调用下一个 agent**
   当你需要将任务传递给 PM 时，使用 agent-team 工具（或 subagent 工具）内部调用。

   **注意**：如果 agent-team 工具不工作，则发送消息提醒用户手动 @PM。

3. **发送工作流状态卡片**
   每次任务传递给下一个角色后，发送工作流状态卡片：
   ```
   📍 工作流进度
...
entry (自动):
  1. 识别任务类型 = 快速调研
  2. 调用 agent-team 工具 → 触发 PM
  3. 发送工作流状态卡片

PM (自动):
  1. 收到任务，分解
  2. 调用 agent-team 工具 → 触发 调研员
  3. 发送工作流状态卡片

调研员 (自动):
  1. 完成任务
  2. 调用 agent-team 工具 → 触发 文档
  3. 发送工作流状态卡片
