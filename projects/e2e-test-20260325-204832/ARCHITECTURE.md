 ...
 * 测试用例: 入口处理
 */
export async function testEntryPoint(projectPath, collector) {
    ...
    try {
        ...
        assert(state.currentStage !== null, '工作流已启动');
        
        collector.record('EP-001', '入口处理测试', true);
    } catch (error) {
        collector.record('EP-001', '入口处理测试', false, error.message);
    }
}
```
...
| 用例ID | 测试项 | 前置条件 | 预期结果 |
|--------|--------|----------|----------|
| RT-001 | entry→pm | entry 完成 | pm 被触发 |
| RT-002 | pm→product | pm 完成 | product 被触发 |
| RT-003 | product→architect | product 完成 | architect 被触发 |
| RT-004 | architect→developer | architect 完成 | developer 被触发 |
| RT-005 | developer→qa | developer 完成 | qa 被触发 |
| RT-006 | qa→writer | qa 完成 | writer 被触发 |
...
export async function testStateManagement(projectPath, collector) {
    ...
    try {
        ...
        // 验证当前阶段
        const validStages = ['入口处理', '项目管理', '需求分析', '架构设计', '开发实施', '测试验证', '文档整理'];
        assert(validStages.includes(state.currentStage), '当前阶段有效');
        
        // 验证下一步角色
        const validRoles = ['entry', 'pm', 'product', 'architect', 'developer', 'qa', 'writer'];
        assert(validRoles.includes(state.nextRole), '下一步角色有效');
        
        // 验证任务状态一致性
        state.tasks.forEach(task => {
            const validStatuses = ['待开始', '处理中', '已完成', '失败'];
            assert(validStatuses.includes(task.status), `任务 ${task.name} 状态有效`);
        });
...
| 用例ID | 测试项 | 预期结果 |
|--------|--------|----------|
| E2E-001 | 完整流程 | Entry → PM → Product → Architect → Developer → QA → Writer 全流程成功 |
...
export async function testCompleteWorkflow(projectPath, collector) {
    const verifier = new StateVerifier();
    const roles = ['entry', 'pm', 'product', 'architect', 'developer', 'qa', 'writer'];
    ...
    try {
        ...
        // 验证当前阶段与已完成任务数一致
        const stages = ['入口处理', '项目管理', '需求分析', '架构设计', '开发实施', '测试验证', '文档整理'];
...
import { testEntryPoint, testTaskDistribution, testRoleTransition, testStateManagement, testTriggerMechanism, testCompleteWorkflow } from './test-cases.js';
...
async function main() {
    ...
    try {
        ...
        // 2. 执行测试用例
        await testEntryPoint(projectPath, collector);
...
## 测试概览
- 测试时间: 2026-03-26 17:14:00
- 项目: e2e-test-20260325-204832
- 总用例数: 13
- 通过数: 13
- 失败数: 0
- 通过率: 100.00%
...
### 角色流转测试
- ✅ RT-001: entry→pm 流转测试
- ✅ RT-002: pm→product 流转测试
- ✅ RT-003: product→architect 流转测试
- ✅ RT-004: architect→developer 流转测试
- ✅ RT-005: developer→qa 流转测试
- ✅ RT-006: qa→writer 流转测试
...
```
Entry（入口处理）→ PM（项目管理）→ Product（需求分析）→ Architect（架构设计）→ Developer（开发实施）→ QA（测试验证）→ Writer（文档整理）
```
